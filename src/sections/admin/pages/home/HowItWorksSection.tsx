...
type FormValues = AdminApiInputs["pages"]["home"]["updateHowItWorksSection"];
...
const HowItWorksSection = () => {
  ...
  const methods = useForm({
    defaultValues,
    resolver: zodResolver(homeSchema.updateHowItWorksSection),
  });
  const { handleSubmit, reset } = methods;

  const { mutate, isLoading: isMutating } =
    adminApi.pages.home.updateHowItWorksSection.useMutation();
    ...
    mutate(data, {
      onSuccess({ data }) {
        ...
        utils.pages.home.getRecord.setData(undefined, (record) =>
          !record ? undefined : { ...record, HowItWorksSection: data },
        );
      },
    });

  useEffect(() => {
    if (data?.howItWorkSection) reset(data.howItWorkSection);
  }, [data, reset]);
