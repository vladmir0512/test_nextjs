...
type FormValues = AdminApiInputs["section"]["privacyPolicy"];
const PrivacyPolicy: NextPageWithLayout = () => {
  ...
  const methods = useForm({
    defaultValues,
    resolver: zodResolver(sectionSchema.privacyPolicy),
  });
  const { getValues, reset, setValue, handleSubmit } = methods;

  const { mutate, isLoading: isSubmitting } =
    adminApi.section.privacyPolicy.useMutation();
  const onSubmit = (formData: FormValues) =>
    mutate(formData, {
      onSuccess({ data }) {
        utils.section.getSection.setData(undefined, (val) =>
          val ? { ...val, privacyPolicy: data } : val,
        );
        stopEditing();
      },
    });
  ...
  useEffect(() => {
    if (data) reset(data.privacyPolicy);
  }, [data, reset]);
  ...
  return (
    <Page title="Privacy Policy">
      <HeaderBreadcrumbs
        ...
        links={[{ name: "Manage Section" }, { name: "Privacy Policy" }]}
      />
...
PrivacyPolicy.getLayout = (page) => <Layout variant="admin">{page}</Layout>;
