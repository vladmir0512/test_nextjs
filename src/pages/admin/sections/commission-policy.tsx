...
type FormValues = AdminApiInputs["section"]["commissionPolicy"];
const CommissionPolicy: NextPageWithLayout = () => {
  ...
  const methods = useForm({
    defaultValues,
    resolver: zodResolver(sectionSchema.commissionPolicy),
  });
  const { getValues, reset, setValue, handleSubmit } = methods;

  const { mutate, isLoading: isSubmitting } =
    adminApi.section.commissionPolicy.useMutation();
  const onSubmit = (formData: FormValues) =>
    mutate(formData, {
      onSuccess({ data }) {
        utils.section.getSection.setData(undefined, (val) =>
          val ? { ...val, commissionPolicy: data } : val,
        );
        stopEditing();
      },
    });
  ...
  useEffect(() => {
    if (data) reset(data.commissionPolicy);
  }, [data, reset]);
...
CommissionPolicy.getLayout = (page) => <Layout variant="admin">{page}</Layout>;
