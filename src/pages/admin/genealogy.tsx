...
import { adminApi, type UserApiOutputs } from "@/utils/api";
import {
  Box,
  Card,
  CardContent,
...
const Genealogy: NextPageWithLayout = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = adminApi.network.genealogy.useQuery(undefined, {
    staleTime: Infinity,
        ...
        chart
          ...
          .onNodeClick((nodeId) => {
            ...
            if (isValid) {
              ...
              window.open(
                `${USER_PATH.register}?referral_id=${String(
                  user?.userId,
                )}&placement_id=${String(parentId)}&placement=${
...
Genealogy.getLayout = (page) => <Layout variant="admin">{page}</Layout>;
